'''Large-angle (DPSI = 90 deg) composite CD&R figure — CDaRR_git.

Pure-plotting: reads the sanitised runs in ``data/large_angle_runs.pkl`` and the
pair chosen by ``regenerate_data.py`` in ``data/selected_pairs.json`` (run that
first). Emits the composite for the ftr_wins case — Past-CPA & Prob-FTR lose
separation while deterministic FTR stays safe. strategy = column, rows =
trajectory / actual-distance / projected-CPA. Figures → figures/.

Run::

    python pairwise_hor_conflict_large_angle_composite.py [--latex]
'''
import json
import math
import os
...
with open(PAIRS_PATH) as f:
    entry = json.load(f)["cases"]["large_angle"]["ftr_wins"]
